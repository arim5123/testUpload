    ...
    <div id="page-content-wrapper">
        ...
        <div class="container" style="margin-top: 50px">
            ...
            <form id="board_notice" class="form-inline" name="fname" method="POST">
                ...
                <table class="intro">
                    ...
                    <tr>
                        <td>(역사)년도</td>
                        <td><input type="text" id="title" name="title" class="form-control-file border" style="width: 10%;" value="<?=$title?>"/>년대</td>
                    </tr>
                    <tr>
                        <td>첨부파일<br>(다중이미지)</td>
                        <td>
                ...
                <p style="text-align:center">
                    <a href="/admin/gallery/history"><button type="button" class="btn btn-danger">취소</button></a> &nbsp;
        ...
        <script>
            ...
            Dropzone.options.dropzone = {
                url: '/admin/gallery/history_modify_upload',
                ...
                init: function () {
                    ...
                    for(var i=0;i<file_val.length;i++){
                        var real_name = file_val[i]['real_name'];
                        var file_name = file_val[i]['file_name'];
                        var base_path = "/img/gallery/history/";
                        ...
                        if(myDropzone.getQueuedFiles().length == 0){
                            ...
                            $.ajax({
                                ...
                                success:function(rs){
                                    console.log(rs);
                                    if(rs === "success"){
                                        Swal.fire({title : '수정되었습니다.', icon: 'success'}).then((result) => { location.replace('/admin/gallery/history'); });
                                    }else{
                                        Swal.fire({title : '작업이 완료되지 않았습니다.', text:'다시 확인해주세요.', icon: 'error'}).then((result) => { location.replace('/admin/gallery/history'); });
                                    }
                                }
                            });
                        }
                ...
                complete:function(rs){
                    if(rs['status'] == "success"){
                        Swal.fire({title : '등록되었습니다.', icon: 'success'}).then((result) => { location.replace('/admin/gallery/history'); });
                    }else{
                        Swal.fire({title : '작업이 완료되지 않았습니다.', text:'다시 확인해주세요.', icon: 'error'}).then((result) => { location.replace('/admin/gallery/history'); });
                    }
                }
